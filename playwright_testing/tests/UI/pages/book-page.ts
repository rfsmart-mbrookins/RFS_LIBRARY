...
export class BookPage {
  ...
  async verifyBookInTable(bookData: {
    ...
    try {
      const bookRow = this.booksTable.locator(
        `tr:has-text("${bookData.title}")`
      );
      await expect(
        bookRow.locator(`td:has-text("${bookData.author}")`)
      ).toBeVisible();
      await expect(
        bookRow.locator(`td:has-text("${bookData.genre}")`)
      ).toBeVisible();
      await expect(
        bookRow.locator(`td:has-text("${bookData.status}")`)
      ).toBeVisible();
      if (bookData.notes) {
        await expect(
          bookRow.locator(`td:has-text("${bookData.notes}")`)
        ).toBeVisible();
      }
    } catch (error) {
